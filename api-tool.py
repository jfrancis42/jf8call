...
"""
jf8call-api — command-line tool for the JF8Call WebSocket API.

Usage:
  api-tool.py [--host HOST] [--port PORT] <command> [args...]

Examples:
  api-tool.py status
  api-tool.py config get
  api-tool.py config set callsign=W5XYZ grid=DM79AA
  api-tool.py send "W4ABC HELLO"
  api-tool.py send "W4ABC HELLO" --submode fast
  api-tool.py tx hb
  api-tool.py tx snr W4ABC
  api-tool.py tx info W4ABC
  api-tool.py tx grid W4ABC
  api-tool.py tx status W4ABC
  api-tool.py tx hearing W4ABC
  api-tool.py tx queue
  api-tool.py tx clear
  api-tool.py radio get
  api-tool.py radio connect --model 3073 --port /dev/ttyUSB0 --baud 9600 --ptt 1
  api-tool.py radio disconnect
  api-tool.py radio freq 14078.0
  api-tool.py radio ptt on
  api-tool.py radio ptt off
  api-tool.py audio devices
  api-tool.py audio restart
  api-tool.py messages [--offset N] [--limit N]
  api-tool.py messages clear
  api-tool.py spectrum
  api-tool.py monitor              # stream all events until Ctrl-C
  api-tool.py monitor --filter message.decoded
"""
...
async def cmd_messages(ws, args):
    ...
    if r["ok"]:
        ...
        total = r["data"].get("total", len(msgs))
...
async def cmd_send(ws, args):
    ...
    if r["ok"]:
        print(f"Queued {r['data'].get('frames')} frame(s),"
              f" {r['data'].get('queue_size')} total in queue")
    else:
        print(f"Error: {r.get('error')}", file=sys.stderr)
...
async def cmd_tx_queue(ws, _args):
    ...
    if r["ok"]:
        ...
        for i, f in enumerate(q):
            print(f"  [{i}] submode={f.get('submode')} frameType={f.get('frameType')}"
